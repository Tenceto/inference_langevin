...
logger_file = "tuning_boot_init.log"
...
# Save results
output_file = f"outputs/bootinit_graphtype_{graph_type}_seed_{seed}_theta_range_{(theta_min, theta_max)}_filter_{h_theta.__name__}.csv"
            ...
            try:
                ...
                for obs_ratio in obs_ratio_list:
                    ...
                    for n_samples_init, margin in zip(n_samples_grid, margin_grid):
                        A_init_boot, _ = boot_init.initial_estimation(Y=this_Y, l1_penalty=0.0, bootstrap_samples=n_samples_init, margin=margin)

                        a_pred = A_init_boot[unknown_idxs[0], unknown_idxs[1]].cpu().numpy()
