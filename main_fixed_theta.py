...
# Save results
output_file = f"outputs/{graph_type}_{seed}_{theta}_{sigma_e}_{p_unknown}_{temperature}_{n_list}.csv"
    ...
    with redirect_stdout(logging):
        for seed, A in enumerate(adj_matrices):
            try:
                ...
                X, Y, A_nan = simulate_data(A, n_list[-1])
                ...
                langevin_likelihood_est = lang.LangevinEstimator(h_theta=ut.heat_diffusion_filter,
                                                                ...
                                                                theta_fixed=theta,
                                                                sigma_e=sigma_e)
                adam_est = lang.AdamEstimator(h_theta=partial(ut.heat_diffusion_filter, theta=theta), 
                                            sigma_e=sigma_e, 
                                            lr=lr, n_iter=n_epochs)
                ...
                for n in n_list:
                    ...
                    A_adam, _ = adam_est.adam_estimate(A_nan, this_X, this_Y)
                    langevin_posterior_est.langevin_estimate(A_nan=A_nan, X=this_X, Y=this_Y, 
                                        sigmas_sq=sigmas ** 2, epsilon=epsilon, steps=steps, temperature=temperature,
                                        projection_method="copy", clip_A_tilde=True, true_A=A)
                    langevin_likelihood_est.langevin_estimate(A_nan=A_nan, X=this_X, Y=this_Y,
                                            sigmas_sq=sigmas ** 2, epsilon=epsilon, steps=steps, temperature=temperature,
                                                projection_method="copy", clip_A_tilde=True, true_A=A)
                    
                    A_all = {
                        "langevin_posterior": langevin_posterior_est.A_tilde, 
                        "langevin_likelihood": langevin_likelihood_est.A_tilde,
                        "adam": A_adam,
                    }
                    ...
                    this_output["real_values"] = A[unknown_idxs[0], unknown_idxs[1]].cpu().numpy().tolist()
                    for method, A_est in A_all.items():
                        this_output[f"pred_{method}"] = A_est[unknown_idxs[0], unknown_idxs[1]].cpu().numpy().tolist()
